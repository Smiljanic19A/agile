  ...
  <section id="writing" class="blog section section--teal" aria-labelledby="blog-title">
    <div class="container">
      ...
      <div class="blog__grid is-desktop-only fade-up">
        ...
        <div class="blog__row">
          <BlogCard v-for="article in standardArticles" :key="article.id" :item="article" variant="standard" />
        </div>
        <div class="blog__text">
          <BlogCard v-for="article in textArticles" :key="article.id" :item="article" variant="text" />
        </div>
      </div>

      <div class="blog__rail snap-rail is-mobile-only fade-up">
        <BlogCard v-for="article in articles" :key="article.id" :item="article" variant="standard" />
      </div>
...
.blog__row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 18px; }
.blog__text { display: grid; grid-template-columns: repeat(2, 1fr); gap: 18px; }

@media (max-width: 1100px) {
  .blog__head { grid-template-columns: 1fr; gap: 28px; margin-bottom: 44px; align-items: start; }
  .blog__row { grid-template-columns: repeat(2, 1fr); }
}
@media (max-width: 720px) {
  .blog__head { margin-bottom: 28px; }
  .blog__lead { font-size: 15px; }

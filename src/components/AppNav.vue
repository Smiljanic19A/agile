...
const onKey = (e) => { if (e.key === 'Escape' && open.value) open.value = false }
  ...
  <header class="nav" :class="{ 'is-scrolled': scrolled }">
    <div class="nav__inner container">
      <a href="#top" class="nav__brand" aria-label="Agile Periodization — home">
        ...
        <span class="nav__wordmark"><strong>Agile</strong> Periodization</span>
      </a>
  ...
  <Transition name="menu">
    <div v-if="open" class="menu" role="dialog" aria-modal="true" aria-label="Menu">
      ...
      <nav class="menu__links container" aria-label="Mobile primary">
        <a v-for="(l, i) in links" :key="l.href" :href="l.href" class="menu__link" :style="{ '--i': i }" @click="close">
...
.nav__inner {
  display: grid; grid-template-columns: 1fr auto 1fr; align-items: center; height: 72px; gap: 24px;
}

.nav__brand {
  display: inline-flex; align-items: center; gap: 10px;
  font-family: var(--font-body); font-size: 14px; letter-spacing: 0.02em; color: currentColor;
}
.nav__wordmark { font-weight: 400; opacity: 0.92; }
.nav__wordmark strong { font-weight: 600; }

.nav__links { display: none; gap: 28px; justify-self: center; }
.nav__link {
  font-size: 14px; font-weight: 500; letter-spacing: 0.01em; color: currentColor;
  opacity: 0.78; transition: opacity 200ms var(--ease); position: relative; padding: 8px 0;
}
.nav__link:hover { opacity: 1; }
.nav__link::after {
  content: ''; position: absolute; left: 0; right: 0; bottom: 4px; height: 1px;
  background: currentColor; transform: scaleX(0); transform-origin: center; transition: transform 220ms var(--ease);
}
...
.nav__cta-arrow { transition: transform 220ms var(--ease); }
.nav__cta:hover .nav__cta-arrow { transform: translateX(3px); }

.nav__burger { justify-self: end; width: 40px; height: 40px; position: relative; border-radius: 999px; }
.nav__burger:hover { background: rgba(247, 242, 233, 0.06); }
.nav__burger-line {
  position: absolute; left: 10px; right: 10px; height: 1.5px;
  background: currentColor; border-radius: 1px; transition: transform 260ms var(--ease), opacity 180ms var(--ease);
}
.nav__burger-line:nth-of-type(1) { top: 20px; }
.nav__burger-line:nth-of-type(2) { top: 20px; }
.nav__burger-line:nth-of-type(1).is-open { transform: rotate(45deg); }
.nav__burger-line:nth-of-type(2).is-open { transform: rotate(-45deg); }

.menu {
  position: fixed; inset: 0; z-index: 70; background: var(--teal); color: var(--cream);
  display: flex; flex-direction: column; overflow-y: auto;
  padding-top: env(safe-area-inset-top); padding-bottom: env(safe-area-inset-bottom); isolation: isolate;
}
.menu__hex { position: absolute; inset: 0; pointer-events: none; z-index: 0; }
.menu > :not(.menu__hex) { position: relative; z-index: 1; }

.menu__head {
  display: flex; align-items: center; justify-content: space-between; height: 60px; flex-shrink: 0;
}
.menu__brand { display: inline-flex; align-items: center; gap: 10px; color: currentColor; font-size: 13px; }
.menu__wordmark { opacity: 0.94; }
.menu__wordmark strong { font-weight: 600; }
.menu__close {
  width: 44px; height: 44px; margin-right: -10px; display: grid; place-items: center;
  color: currentColor; border-radius: 999px; transition: background 200ms var(--ease);
}
.menu__close:hover { background: rgba(247, 242, 233, 0.08); }
...
.menu__link {
  ...
  padding: 14px 0; color: var(--cream); text-decoration: none;
...
.menu__link:hover { opacity: 0.72; }
.menu__link-index { font-family: var(--font-mono); font-size: 11px; letter-spacing: 0.08em; color: rgba(247,242,233,0.5); text-transform: uppercase; }
.menu__link-label { font-family: var(--font-display); font-size: clamp(36px, 9vw, 56px); font-weight: 700; letter-spacing: -0.02em; line-height: 1; }

@keyframes menuLinkIn { to { opacity: 1; transform: none; } }
@media (prefers-reduced-motion: reduce) { .menu__link { animation: none; opacity: 1; transform: none; } }
...
.menu-enter-active, .menu-leave-active { transition: opacity 280ms var(--ease), transform 320ms var(--ease); }
...
@media (max-width: 899px) {
  ...
  .nav.is-scrolled { background: var(--teal); backdrop-filter: none; -webkit-backdrop-filter: none; }
}
@media (max-width: 380px) { .nav__wordmark { font-size: 12px; } .nav__logo svg { width: 18px; height: 18px; } }
</style>

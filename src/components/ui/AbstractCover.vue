...
const palette = computed(() => {
  if (props.tone === 'cream') {
    return { bg: '#F3F3F3', ink: '#0E1A1A', teal: '#457c77', tealDeep: '#2a5854', tealSoft: '#6fa5a0', tealMist: 'rgba(69,124,119,0.16)' }
  }
  return { bg: '#2a5854', ink: '#F3F3F3', teal: '#457c77', tealDeep: '#1B4A46', tealSoft: '#6fa5a0', tealMist: 'rgba(247,242,233,0.14)' }
})
    ...
    <svg viewBox="0 0 400 300" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
      ...
      <template v-else-if="variant === 4">
        <g :stroke="palette.tealSoft" fill="none" stroke-width="1" opacity="0.85">
          <path d="M -20 220 Q 100 160 200 200 T 420 180" /><path d="M -20 240 Q 100 180 200 220 T 420 200" />
          <path d="M -20 260 Q 100 200 200 240 T 420 220" /><path d="M -20 280 Q 100 220 200 260 T 420 240" />
        </g>
        <g :stroke="palette.ink" fill="none" stroke-width="1.2"><path d="M -20 200 Q 100 140 200 180 T 420 160" /></g>
      ...
      <template v-else-if="variant === 5">
        <g :fill="palette.ink" opacity="0.7">
          <template v-for="r in 6" :key="`r${r}`">
            <template v-for="c in 9" :key="`c${r}-${c}`"><circle :cx="60 + (c - 1) * 35" :cy="80 + (r - 1) * 30" r="2" /></template>
          </template>
        </g>
...
<style scoped>
.cover { position: relative; width: 100%; aspect-ratio: 4 / 3; overflow: hidden; border-radius: var(--radius); }
.cover svg { width: 100%; height: 100%; display: block; }
.cover__meta {
  position: absolute; inset: auto 16px 14px 16px;
  display: flex; justify-content: space-between; align-items: flex-end;
  font-family: var(--font-mono); font-size: 11px; letter-spacing: 0.06em; text-transform: uppercase;
  color: currentColor; opacity: 0.62; pointer-events: none;
}
@media (max-width: 720px) { .cover { aspect-ratio: 16 / 9; } }
</style>

...
<template>
  <a :href="item.url" target="_blank" rel="noopener noreferrer" class="card" :class="[`card--${variant}`]">
    ...
    <div class="card__body">
      ...
      <h3 class="card__title" :class="variant === 'feature' ? 'display-3' : ''">{{ item.title }}</h3>
      <p v-if="item.description && variant !== 'text'" class="card__dek">{{ item.description }}</p>
      <div class="card__foot">
        <span class="card__date">{{ item.date }}</span>
        <span class="card__read">Read essay <span class="card__arrow" aria-hidden="true">→</span></span>
      </div>
    </div>
  </a>
</template>
...
.card--feature { display: grid; grid-template-columns: 1.05fr 1fr; align-items: stretch; gap: 36px; padding: 28px; }
.card--feature .card__cover { margin-bottom: 0; }
.card--feature .card__body { display: flex; flex-direction: column; justify-content: center; }
.card--feature .card__title { font-size: clamp(28px, 3.4vw, 48px); margin-bottom: 18px; }
.card--feature .card__dek { font-size: 17px; line-height: 1.55; max-width: 44ch; margin-bottom: 36px; }

.card--text { background: transparent; border: 1px solid var(--hairline-light); padding: 28px; }
.card--text .card__title { font-size: clamp(22px, 2vw, 26px); margin-bottom: 20px; }

@media (max-width: 900px) { .card--feature { grid-template-columns: 1fr; gap: 18px; } }
@media (max-width: 720px) {
  .card, .card--feature { padding: 18px; }
  .card--feature .card__title { font-size: 26px; margin-bottom: 12px; }
  .card--feature .card__dek { font-size: 15px; margin-bottom: 22px; }
  .card--text { padding: 20px; }
  .card__cover { margin-bottom: 16px; }
  .card__title { font-size: 20px; }
  .card__dek { margin-bottom: 20px; -webkit-line-clamp: 3; -webkit-box-orient: vertical; display: -webkit-box; overflow: hidden; }

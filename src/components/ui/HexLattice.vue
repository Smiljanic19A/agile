...
const props = defineProps({
  ...
  size: { type: Number, default: 80 }, // hex circumradius in viewBox units
...
const palette = computed(() => {
  if (props.tone === 'on-cream') {
    return { stroke: '#0E1A1A', base: 0.06, accent: 0.14, fill: 0.04 }
  }
  if (props.tone === 'on-deep') {
    return { stroke: '#5BA39D', base: 0.16, accent: 0.34, fill: 0.08 }
  }
...
// Grid pattern tile: width = 3R, height = R*sqrt3
const tile = computed(() => {
  const r = props.size
  return { w: 3 * r, h: sqrt3 * r }
})

// Pattern tile contains: full hex at center + 4 quarter-hexes at corners
...
// Generate accent hex positions for grid: pick deterministic cells from a virtual grid.
// Coordinates are in viewBox space.
const gridAccents = computed(() => {
  if (props.variant !== 'grid') return []
  const r = props.size
  const rand = mulberry32(props.seed)
  // virtual grid roughly the size of typical hero/newsletter
  const cols = 18
  const rows = 12
...
// Cluster variant: 7-cell honeycomb (center + 6 neighbours)
const clusterCells = computed(() => {
  ...
  while (accent.size < Math.min(props.accents, offsets.length - 2)) {
    accent.add(Math.floor(rand() * offsets.length))
  }
  return offsets.map(([x, y], i) => ({
    key: `c${i}`,
    x,
    y,
...
// Ring variant: 6 hexes arranged in an orbit
const ringCells = computed(() => {
  ...
  return Array.from({ length: 6 }, (_, i) => {
    ...
    return { key: `r${i}`, x, y, points: hexPoints(x, y) }
  })
})
...
const viewBox = computed(() => {
  ...
  if (props.variant === 'grid') {
    // grid spans an arbitrary canvas; the SVG will slice to fill its container
    return '0 0 1200 800'
  }
...
const preserveAspect = computed(() =>
  props.variant === 'grid' ? 'xMidYMid slice' : 'xMidYMid meet'
)
...
<template>
  ...
  >
    <defs>
      ...
      >
        <polygon
          :points="tilePoints.center"
          fill="none"
          :stroke="palette.stroke"
          :stroke-opacity="palette.base"
          :stroke-width="strokeWidth"
          stroke-linejoin="round"
        />
        <polygon
          :points="tilePoints.tl"
          fill="none"
          :stroke="palette.stroke"
          :stroke-opacity="palette.base"
          :stroke-width="strokeWidth"
    ...
    <g :mask="mask !== 'none' ? `url(#${uid}-mask)` : undefined">
      ...
      <g v-if="variant === 'grid'" class="hex__accents">
        <polygon
          v-for="a in gridAccents"
          :key="a.key + '-f'"
          :points="a.points"
          :fill="palette.stroke"
          :fill-opacity="palette.fill"
          :stroke="palette.stroke"
          :stroke-opacity="palette.accent"
          :stroke-width="strokeWidth"
          stroke-linejoin="round"
        />
      </g>
      ...
      <g v-if="variant === 'cluster'" class="hex__accents">
        <polygon
          v-for="c in clusterCells"
          :key="c.key"
          :points="c.points"
          :fill="c.accent ? palette.stroke : 'none'"
          :fill-opacity="c.accent ? palette.fill : 0"
          :stroke="palette.stroke"
          :stroke-opacity="c.accent ? palette.accent : palette.base"
          :stroke-width="strokeWidth"
          stroke-linejoin="round"
        />
      </g>

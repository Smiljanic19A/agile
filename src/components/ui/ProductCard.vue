...
<template>
  <a :href="item.url" target="_blank" rel="noopener noreferrer" class="prod">
    <div class="prod__cover">
      <img v-if="item.image" :src="item.image" :alt="item.title" class="prod__img" />
      <AbstractCover v-else :seed="item.cover?.seed ?? 1" tone="cream" :label="item.type" />
    </div>
    <div class="prod__body">
      ...
      <div class="prod__foot">
        <span class="prod__cta">View on {{ sourceName }} <span class="prod__arrow" aria-hidden="true">→</span></span>
      </div>
    </div>
  </a>
</template>
...
.prod__img { width: 100%; aspect-ratio: 4/3; object-fit: cover; border-radius: var(--radius); display: block; }
...
@media (max-width: 720px) {
  .prod { padding: 16px; }
  .prod__cover { margin-bottom: 16px; }
  .prod__title { font-size: 20px; }
  .prod__blurb { margin-bottom: 16px; -webkit-line-clamp: 3; -webkit-box-orient: vertical; display: -webkit-box; overflow: hidden; }
  .prod__foot { padding-top: 12px; }
}
</style>
